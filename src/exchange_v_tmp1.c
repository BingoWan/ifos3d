...
double exchange_v(float*** vx, float*** vy, float*** vz,
  ...
  extern int NX, NY, NZ, POS[4], NPROCX, NPROCY, NPROCZ, BOUNDARY, FDORDER, INDEX[7], ABS_TYPE; /*MYID,LOG,*/
		...
		for (l=FDORDER/2-1;l>=1;l--) {
		    for (i=1;i<=NX;i++) {
				...
				for (k=1;k<=NZ;k++) {
			    	...
			    	Fbt[i * slice_bt + k * strip_bt + n + 1] = Fv[3 * (idx_v + (NY -2 * l  + 1) * slice) + 0];
        ...
        for (l=FDORDER/2-1;l>=1;l--) {
		    for (i=1;i<=NX;i++) {
			    ...
			    for (k=1;k<=NZ;k++) {
			    	...
			    	Fv[3 * (idx_v + (1 -2 * l) * slice) + 0] = Fbt[i * slice_bt + k * strip_bt + n + 1];
